...
class ProductProvider extends Component {
    ...
    addToCart = (id) => {
        let tempProoducts = [...this.state.products];
        const index = tempProoducts.indexOf(this.getItem(id));
        const product = tempProoducts[index];
        product.inCart = true;
        product.count = 1;
        const price = product.price;
        product.total = price;
        this.setState({ product: tempProoducts, cart: [...this.state.cart, product] }, () => this.addTotals())
    }
    openModal = id => {
        const product = this.getItem(id);
    ...
    addTotals = () => {
        ...
        const tempTax = parseFloat((subTotal * 0.1).toFixed(2));
